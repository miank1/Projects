...
namespace DBHelper
{
    ...
    public sealed class MySqlHelper
    {
       ...
       // Added by Maha to return dataset for ETL import Audit Trail

        public static DataSet UpdateDataset_WithDataset(DataSet ds, string constr)
        {
            string TableName;
            ...
            sqlConnection1 = new MySqlConnection();
            ...
            try
            {
                ...
                for (int i = 0; i < icount; i++)
                {
                    ...
                    bool isView = false;
                    //if (dt.ExtendedProperties.ContainsKey(NTrust.Data.DAL.EXT_PROP_ISVIEW))
                    //{
                    //    isView = (bool)dt.ExtendedProperties[NTrust.Data.DAL.EXT_PROP_ISVIEW];
                    //}
                    if (isView == false)
                    {
                        TableName = ds.Tables[i].TableName;
                        da = new MySqlDataAdapter();
                        sqlSelectCommand1 = new MySqlCommand();
                        sqlSelectCommand1.CommandText = "Select * from " + TableName;
                        sqlSelectCommand1.Connection = sqlConnection1;
                        sqlSelectCommand1.Transaction = trans;
                        da.SelectCommand = sqlSelectCommand1;
                        cmdb = new MySqlCommandBuilder(da);
                        cmdb.QuotePrefix = "";
                        cmdb.QuoteSuffix = "";
                        da.DeleteCommand = cmdb.GetDeleteCommand();
                        da.DeleteCommand.CommandText = da.DeleteCommand.CommandText.Substring(0,
                            da.DeleteCommand.CommandText.ToUpper().IndexOf("?ORIGINAL_PKEY")) + " ?Original_PKEY";

                        da.UpdateCommand = cmdb.GetUpdateCommand();
                        da.UpdateCommand.CommandText = da.UpdateCommand.CommandText.Substring(0,
                            da.UpdateCommand.CommandText.ToUpper().IndexOf("?ORIGINAL_PKEY")) + " ?Original_PKEY";
        ...
        public static DataTable ExecuteDataTable(string connectionString, CommandType commandType, string commandText)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                //PrepareCommand(cmd, connection, (MySqlTransaction)null, commandType, commandText, (commandParameters)null, out mustCloseConnection );
        ...
        // Added by RameshV
        /// <summary>
        /// Execute a MySqlCommand with Transaction (that returns a resultset and takes no parameters) against the database specified in 
        /// the connection string. 
        /// </summary>
        public static DataTable ExecuteDataTable(MySqlTransaction transaction, string connectionString, CommandType commandType, string commandText)
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand();
                bool mustCloseConnection = true;
                PrepareCommand(cmd, transaction.Connection, transaction, commandType, commandText, (MySqlParameter[])null, out mustCloseConnection);
        ...
        public static void ExecuteDataTable(ref DataTable dataTbl, string connectionString, string strSelectSql)
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("ConnectionString");
            if (dataTbl == null) throw new ArgumentNullException("DataTable");
        ...
        public static void UpdateDataset(DataSet ds, string constr)
        {
            string TableName;
            MySqlCommand sqlSelectCommand1;
            MySqlDataAdapter da = null;
        ...
        public static void UpdateDataset(DataSet ds, string constr, bool blnDelete)
        {
            string TableName;
            MySqlCommand sqlSelectCommand1;
            ...
            try
            {
                ...
                for (int i = icount - 1; i >= 0; i--)
                {
                    ...
                    if (isView == false)
                    {
                        ...
                        cmdb.QuotePrefix = "[";
                        cmdb.QuoteSuffix = "]";
                        sqlInsertCommand1 = cmdb.GetInsertCommand();
                        sqlUpdateCommand1 = cmdb.GetUpdateCommand();
                        sqlDeleteCommand1 = cmdb.GetDeleteCommand();
                        da.Update(ds, TableName);
                    }
                }
                trans.Commit();
            ...
            //				DataRow drCopy;
            //				DataRow drCurrent ; 
            //				drCopy = ds.Tables[dbcex.Row.Table.TableName.ToString()].NewRow();
            //				
            //				drCurrent =   ds.Tables[dbcex.Row.Table.TableName.ToString()].Rows.Find(dbcex.Row.ItemArray[0].ToString());
            //				object [] myArray = new object[drCurrent.Table.Columns.Count];
            //				int intLoop = 0;
            //				foreach(DataColumn dc in drCurrent.Table.Columns)
            //				{
            //					if(dc.ReadOnly == false)
            //					{
            //						myArray[intLoop]		=drCurrent[dc.ColumnName];
            //						intLoop ++;
            //					}
            //				}
            //				drCopy.ItemArray = myArray;
            //				// 'Get the current values from the database
            //				//UpdateRow("Customer", dbcEx.Row.Item("ID"))
            //				UpdateRow(ds,dbcex.Row.Table.TableName.ToString(),dbcex.Row.ItemArray[0].ToString(),constr);
            //				drCurrent.ItemArray = drCopy.ItemArray;
            //				//'Now restore the values entered by the user and save again
            //				 da.Update(ds.Tables[dbcex.Row.Table.TableName.ToString()]);
            //				
        ...
        private static void UpdateRow(DataSet ds, string strTableName, string strID, string constr)
        {
            //'Get a reference to the specified row

            DataRow dr = ds.Tables[strTableName].Rows.Find(strID);
            MySqlConnection sqlConnection5;
            sqlConnection5 = new MySqlConnection();
            sqlConnection5.ConnectionString = constr;

            //  'Create a Command update to pull the new underlying data
            MySqlCommand cmd = new MySqlCommand("SELECT * FROM " + strTableName + " WHERE PKEY=" + strID.ToString(), sqlConnection5);
            ...
            object[] myArray = new object[dr.Table.Columns.Count];
            int intLoop = 0;
            foreach (DataColumn dc in dr.Table.Columns)
            {
                ...
                {
                    // rdr.);
                    myArray[intLoop] = rdr[dc.ColumnName];
                    intLoop++;
        ...
        #region ExecuteTransNonQuery
        // Added by V.Ramesh
        /// <summary>
        /// Executes the respective command for each inserted, updated, or deleted row in the Table.
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ///  ExecuteTransNonQuery(Connection,QueryString Array);
        /// </remarks>
        /// <param name="connection">A valid MySqlConnection, on which to execute this command</param>
        /// <param name="QueryString">The DataTable used to update the data source.</param>


        public static void ExecuteTransNonQuery(string constr, string[] Query)
        {
            ...
            try
            {
                ...
                foreach (string QueryStringvalue in Query)
                {
                    if (QueryStringvalue != null && QueryStringvalue != "")
                    {
                        ExecuteNonQuery(trans, CommandType.Text, QueryStringvalue);
                    }
        ...
        public static int[] ExecuteTransNonQueryWithIDReturn(string constr, string[] Query)
        {

            List<int> lsteturnPkey = new List<int>();
            ...
            try
            {
                ...
                foreach (string QueryStringvalue in Query)
                {
                    if (QueryStringvalue != null && QueryStringvalue != "")
                    {
                        lsteturnPkey.Add((ExecuteNonQuery(trans, CommandType.Text, QueryStringvalue)));
                    }
            ...
            return lsteturnPkey.ToArray();
        }
        #endregion
        #region ExecuteSeqNumberTransQuery
        // Added by V.Ramesh
        /// <summary>
        /// Executes the respective command for each inserted, updated, or deleted row in the Table.
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ///  ExecuteSeqNumberTransQuery(Connection,QueryString Array);
        /// </remarks>
        /// <param name="connection">A valid MySqlConnection, on which to execute this command</param>
        /// <param name="QueryString">The Query used to update the database.</param>


        public static DataTable ExecuteSeqNumberTransQuery(string constr, string[] Query)
        {
            DataTable dt = new DataTable();
            MySqlConnection sqlConnection1;
            sqlConnection1 = new MySqlConnection();
            ...
            try
            {
                dt = ExecuteDataTable(trans, constr, CommandType.Text, Query[0]);
                ExecuteNonQuery(trans, CommandType.Text, Query[1]);
                trans.Commit();
                sqlConnection1.Close();
