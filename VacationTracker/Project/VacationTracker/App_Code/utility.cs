    ...
    public class CUtility
    {
        ...
        public static bool SendNotificationMailToManager(string AssociateName, string ManagerEmailId, DateTime FromDate, DateTime Todate, string vacationTitle, string vacationDescription)
        {
            try
            {
                ...
                str.AppendLine(string.Format("DTSTART:{0:yyyyMMddTHHmmssZ}", FromDate.AddHours(-5).AddMinutes(-30)));//TimeZoneInfo.ConvertTimeToUtc("BeginTime").ToString("yyyyMMddTHHmmssZ")));
                str.AppendLine(string.Format("DTSTAMP:{0:yyyyMMddTHHmmssZ}", DateTime.UtcNow));
                str.AppendLine(string.Format("DTEND:{0:yyyyMMddTHHmmssZ}", Todate.AddHours(-5).AddMinutes(-31)));//TimeZoneInfo.ConvertTimeToUtc("EndTime").ToString("yyyyMMddTHHmmssZ")));
